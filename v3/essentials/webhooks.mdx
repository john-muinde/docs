...
- `transaction.status.updated` - _(Deprecated)_ Generic transaction status update
...
- `refund.completed` - Crypto refund successfully sent back to integrator (transaction status → `REVERSED`)
- `refund.failed` - Refund exhausted all retry attempts — manual intervention required
- `refund.lightning.invoice_needed` - Lightning offramp needs a bolt11 refund invoice submitted
