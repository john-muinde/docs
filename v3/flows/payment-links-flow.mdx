...
<Tip>
If you don't need links generated programmatically, you can create and share one
from the dashboard without writing any code — see [Payment
Links](/v3/essentials/payment-links).
</Tip>

## 1. Create the Payment Link

Call `POST /api/v3/dashboard/payment-links`. At a minimum you'll configure:
...
- `GET /api/v3/dashboard/payment-links/transactions/reference/{referenceId}` — status of a specific payment
- `GET /api/v3/dashboard/payment-links/{id}/transactions` — all payments for a given link
- `GET /api/v3/dashboard/payment-links/{id}/analytics` — totals for a given link

<Note>
The per-link routes take the link's `id` as returned by the create call, not its
short code.
</Note>
