...
Complete flows for disbursing fiat payments via different methods.
...
**Limits**: Per-transaction and daily limits vary by country

**Key Points**:
- Funds debited IMMEDIATELY when withdrawal created
- If withdrawal fails, funds automatically reversed
- Hash-based duplicate prevention (same minute)
- Automatic retry for failed withdrawals (up to 10 attempts)
...
**Available Countries**: Kenya, South Africa only
...
**Settlement**: Depends on receiving bank

**Key Points**:
- Same immediate debit mechanism
- Longer settlement time than mobile money
- Automatic reversal on failure
- Bank account validation recommended before withdrawal

---

## Lipa na M-Pesa / Buy Goods Withdrawal Flow

Send payments to merchant till numbers.

**Available Country**: Kenya only
...
**Currency**: KES
**Speed**: Instant
**Use Case**: Paying merchants, businesses

**Key Points**:
- No customer record needed (just till number)
- Same immediate debit mechanism
- Instant settlement
- Kenya-specific feature

---

## Paybill Withdrawal Flow

Send payments to paybill numbers with account reference.
...
**Currency**: KES
**Speed**: Instant
**Use Case**: Paying utilities, subscriptions, services

**Key Points**:
- Requires paybill number and account reference
- No customer record needed
- Instant settlement
- Account reference shows on recipient's statement
...
- Polling every 5 minutes for pending
- 24-hour timeout for stuck transactions
- Automatic retry for failures (up to 10 attempts)
...
### Validate Recipients
- Use `/customers/validate-mobile-money` before withdrawals
- Verify bank account details for transfers
- Ensures recipient can receive funds

### Unique References
Always provide unique `referenceId` values.

### Implement Webhooks
Set up webhook endpoint for real-time updates.

### Handle Reversals
Monitor for FAILED/EXPIRED statuses - funds auto-reversed.

### Idempotency
Safe to retry with same `referenceId` if request fails.
...
Use sandbox mode:
- **Test Phone (Success)**: Provided in sandbox docs
- **Test Phone (Failure)**: Triggers failure scenarios
- **Test Bank Account**: Use test accounts
- **Test Amounts**: Specific amounts trigger outcomes
- **Test Till/Paybill**: Test numbers available in sandbox
