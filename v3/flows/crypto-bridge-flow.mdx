...
Crypto Bridge enables cross-border remittances using blockchain networks as the settlement rail. The provider (e.g. Orange Group) initiates a transaction with a single API request that includes a pre-generated crypto payment invoice. Kotani Pay collects source fiat, pays the invoice, and the provider settles destination fiat to the recipient.
...
```mermaid
sequenceDiagram
    participant Provider as Provider (e.g. Orange)
...
sequenceDiagram
    ...
    alt Payout Successful
        ...
        ProviderFiat->>Kotani Pay: POST /webhooks/orange/status
...
```
X-Orange-Signature: HMAC-SHA256(secret, payload)
X-Orange-Timestamp: Unix timestamp
```
...
- **Use both IDs**: Always track `kotani_reference_id` and your own `orange_reference_id` together
