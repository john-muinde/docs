...
**Send phone numbers in international format**, with the country code. National
format is ambiguous — the same digits are a real number in several countries we
support — so we won't guess: without a country we can determine, the response tells
you so rather than validating the wrong corridor. If you can only supply a national
number, send `countryCode` alongside it.
