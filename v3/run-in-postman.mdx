...
<AccordionGroup>
  <Accordion icon="api" title="Complete API Coverage">
  ...
  <Accordion icon="settings" title="Pre-configured Environments">
    - **Sandbox Environment**: `https://sandbox-api.kotanipay.com` - **Production
      Environment**: `https://api.kotanipay.com` - **Environment Variables**: Pre-set for immediate use - **Authentication Tokens**: Automatically managed - **Base URLs**: Ready for both testing and production
  </Accordion>
  <Accordion icon="code" title="Sample Data & Examples">
...
<Steps>
  ...
  <Step title="Configure API Keys">
    Update the environment variables with your API credentials: - `api_key`: Your API key from the authentication flow - `base_url`: Already configured for each environment
  </Step>
...
<Tabs>
  ...
  <Tab title="Deposits">
    1. **Create Customer**: Add a mobile money customer 2. **Initiate Deposit**: Start a deposit transaction 3. **Check Status**: Monitor transaction progress 4. **Handle Webhooks**: Process real-time updates
  </Tab>
  <Tab title="Payouts">
    1. **Verify Balance**: Check wallet balance 2. **Initiate Payout**: Send money to customers 3. **Track Status**: Monitor payout progress 4. **Confirm
